...
- The loader snippet registers four faces: `jf-lanyangming-light` (**200**) and `jf-lanyangming-bold` (**700**) under family alias `jf-lanyangming`; `jf-lanyanghei-bold` (**700**) and `jf-lanyanghei-heavy` (**900**) under alias `jf-lanyanghei`. Registration is a request, not a guarantee: `_setFont` only names, aliases, and weights a face client-side. The face list actually delivered comes from the Justfont project, and the kit mints `@font-face` rules for that list alone.
- What project **65960** serves today is `jf-lanyanghei-bold` and `jf-lanyanghei-heavy` — no Ming (measured 2026-08-21: the kit's `go.justfont.com/jfont/api?…&p=65960&t=woff` response lists exactly those two `fn` entries, for `https://archive.tw` and `http://localhost:5174` alike, so it is project-level rather than a domain allowlist gap; `document.fonts` holds only `jf-lanyanghei 700` and `jf-lanyanghei 900`). Consequence: Han record body has no webfont and falls through `--font-record` to system Ming (measured: `Songti TC`). Read that one API response before blaming CSS for a missing Lanyang face.
