...
- `sql/add-speakers-name-index.sql`（講者 name partial index，加速 photoURL fallback 與列表頁）
- `sql/view_speakers.sql`（view 仍保留於 DB；runtime 已改走 `src/db/speaker-detail.ts` 的 indexed 查詢）
