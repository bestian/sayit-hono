...
const SUBMIT_BUTTON_RE = /<button\b[^>]*\bsayit-search__submit\b[^>]*>([\s\S]*?)<\/button>/gi;
	...
	it('uses sparkle emoji instead of inline SVG in every sayit-search__submit button', () => {
		...
		for (const [path, source] of entries) {
			...
			for (const match of matches) {
				...
				expect(openTag, `${path}: submit button must keep aria-label="Search"`).toContain('aria-label="Search"');
				expect(inner, `${path}: submit button must show ✨`).toContain('✨');
				expect(inner, `${path}: submit button must not use inline magnifying-glass SVG`).not.toMatch(/<svg\b/i);
			}
		}
	});
});
...
describe('search submit markup for site-search forms in view sources', () => {
	it('forbids icon-search on site-search submit controls and requires visible ✨ with Search label', () => {
		...
		for (const [path, source] of entries) {
			...
			expect(source, `${path}: search submit must keep aria-label="Search"`).toContain('aria-label="Search"');
		}
	});
});
